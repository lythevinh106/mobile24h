...
@section('content')
    ...
    <form method="post" action="/admin/post_category/update/{{ $post_category->id }}">


        <div class="mb-3">
            <label for="exampleInputEmail1" class="form-label">Tên Danh Mục</label>
            <input value="{{ $post_category->name }}" type="text" name="name" class="form-control"
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="parent_id" aria-label="Default select example">
                ...
                @foreach ($parents as $parent)
                    <option {{ $parent->id == $post_category->id ? 'disabled' : '' }}
                        {{ $parent->id == $post_category->parent_id ? 'selected' : '' }} value="{{ $parent->id }}">
                        {{ $parent->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-check">
            <input class="form-check-input" type="radio" name="active" value="1"
                {{ $post_category->active == 1 ? 'checked' : '' }} id="flexRadioDefault1">
            <label class="form-check-label" for="">
                Có
            </label>
        </div>
        <div class="form-check">
            <input class="form-check-input" {{ $post_category->active == 0 ? 'checked' : '' }} type="radio"
                name="active" value="0" id="flexRadioDefault2">
